...
MPI_data <- function(){
  ...
  MPI <- openxlsx::read.xlsx("https://ophi.org.uk/wp-content/uploads/Table-8-All-MPI-Data-2010-2022.xlsx",
...
WB_data <- function(url){
  ...
  data <- read.csv(unz(temp,filename), header = T,skip = 3, as.is = T)
  return(data)
}
...
WB_data_long <- function(WBdata) {
  ...
  for (col in unique(WBdata$indicator)) {
    ...
    condition <- sum(is.na(indic_long))/dim(WBdata_long)[1] > 0.50 # TRUE = more than the 70% of the observations are missing values
    if (!condition) {
...
data_join <- function(MPIdata,WBdata,lags){
  data <- MPIdata %>% right_join(WBdata,by=c("iso"="iso","country"="country"))
