...
main_function_dum = function(df, target){
  results = data.frame()
  nfolds <- 5
  ...
  ytrain <- data$data_train[, target]; Xtrain <- data$data_train[,-c(1:7)] #scale
  #ytrain <- (ytrain - mean(ytrain))/sd(ytrain)
  ytest <- data$data_test[, target]; Xtest <- data$data_test[,-c(1:7)]
  ...
  results = data.frame( 
    ...
    "n" = nrow(df),  
    "p"= length(colnames(df)[!((colnames(df) %in% c(colnames(df)[1:7])))]),  
...
main_function_unaobs_dum = function(df, target){
  ...
  ytest <- df_unaobs[, target]; Xtest <- df_unaobs[,-c(1:7)]
  #ytest <- (ytest - mean(ytest))/sd(ytest)
  # Create data frame for prediction
  data <- as.data.frame(cbind(ytrain, Xtrain))
