...
cv.pls2 <- function(X, y, nfolds, max.directions) {
  K <- nfolds
  D <- max.directions
  n <- dim(X)[1]; p <- dim(X)[2] #number of observations and number of predictors
  id <- sample(1:K, n, replace=TRUE, prob=rep(1/K,K))
  ...
  for (d in 1:D){
    for(i in 1:K){
      ...
      # Fit Beta model
      beta.fit <- betareg::betareg("ytrain~." , data = data.frame(ytrain,Pr_train), link.phi = "log", link = "logit"  )
      # Predict over Xtest
      data <- data.frame(ytest,Pr_test); names(data) <- names(data.frame(ytrain,Pr_train))
      
      y_pred_test = predict(beta.fit, newdata = data)
